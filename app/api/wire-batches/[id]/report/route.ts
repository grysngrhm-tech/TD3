...
<head>
  <meta charset="utf-8">
  <title>Wire Transfer Request - ${escapeHtml(builder?.company_name) || 'Builder'}</title>
  <style>
...
<body>
  ...
  <div class="section">
    ...
    <div class="wire-box">
      <div class="bank-name">${escapeHtml(builder?.bank_name) || 'Bank Name Not Provided'}</div>
      <div class="wire-detail">
        <label>Account Name</label>
        <span class="value">${escapeHtml(builder?.bank_account_name) || escapeHtml(builder?.company_name) || 'N/A'}</span>
      </div>
      <div class="wire-detail">
        <label>Routing Number</label>
        <span class="value">${escapeHtml(builder?.bank_routing_number) || 'N/A'}</span>
      </div>
      <div class="wire-detail">
        <label>Account Number</label>
        <span class="value">${escapeHtml(builder?.bank_account_number) || 'N/A'}</span>
      </div>
      <div class="wire-detail">
        <label>Company</label>
        <span class="value">${escapeHtml(builder?.company_name) || 'N/A'}</span>
      </div>
      ${builder?.address_street ? `
      <div class="wire-detail">
        <label>Address</label>
        <span class="value">${escapeHtml(builder.address_street)}, ${escapeHtml(builder.address_city)}, ${escapeHtml(builder.address_state)} ${escapeHtml(builder.address_zip)}</span>
      </div>
      ` : ''}
    </div>
  </div>
  ...
  <div class="section">
    ...
    <table class="draws-table">
      ...
      <tbody>
        ${draws.map((draw: any) => `
        <tr>
          <td>${escapeHtml(draw.project?.project_code) || escapeHtml(draw.project?.name) || 'Unknown Project'}</td>
  ...
  <div class="footer">
    ...
    ${batch.notes ? `<p>Notes: ${escapeHtml(batch.notes)}</p>` : ''}
  </div>
</body>
</html>
`
